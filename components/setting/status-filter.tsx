...
export function StatusFilter<TData, TValue>({
    ...
    const [active, setActive] = useQueryState(filterField, {
        defaultValue: '',
        clearOnDefault: true,
    })
    ...
    const currentActive =
        active === 'true' ? options[0].label : active === 'false' ? options[1].label : undefined
    if (currentActive) {
        ...
        <Popover>
            <PopoverTrigger asChild>
                <Button
                    ...
                    size="sm"
                    className={cn(
                        'min-h-8 h-fit w-full bg-transparent  m-0 p-0 flex justify-start',
                        {
                            'justify-center': align === 'center',
                            'border-zinc-50': selectedValues?.size > 0,
                        },
                        'border rounded-[4px] p-2 border-secondary-200',
                    )}
                >
                    <p className="m-0 p-0 flex items-center justify-start text-zinc-500 font-bold">
                        {/* {title}
                        < */}
